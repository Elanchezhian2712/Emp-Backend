...
class UserController {
  ...
  updateUser = async (req, res, next) => {
    try {
      ...
      // Check if req.user and req.user.type are properly set
      ...
      if (req.user.type === "admin") {
        ...
        if (type) {
          ...
          if (dbUser.type !== type) {
            ...
            // Check if admin is trying to change their own position
            if (_id === id)
              return next(
            ...
            // Check if changing from employee to admin/leader or from leader to admin/employee is valid
            if (
              ...
              if (dbUser.team != null)
                return next(
                  ErrorHandler.badRequest(
                    `Error : ${dbUser.name} is in a team.`
                  )
                );
