...
class TeamController {
  createTeam = async (req, res, next) => {
    ...
    if (!name)
      return next(
        ErrorHandler.badRequest("Required Parameter Teams Name Is Empty")
      );
    const team = {
      name,
      description,
    ...
    if (!teamResp)
      return next(ErrorHandler.serverError("Failed To Create The Team"));
    res.json({
      success: true,
      message: "Team Has Been Created",
      team: new TeamDto(teamResp),
    });
  };
  ...
  updateTeam = async (req, res, next) => {
    ...
    if (leader && !mongoose.Types.ObjectId.isValid(leader))
      return next(ErrorHandler.badRequest("Invalid Leader Id"));
    ...
    return teamResp.modifiedCount != 1
      ? next(ErrorHandler.serverError("Failed To Update Team"))
  ...
  addMember = async (req, res, next) => {
    ...
    if (user.type != "employee")
      return next(ErrorHandler.badRequest(`${user.name} is not an employee`));
    if (user.team)
      return next(ErrorHandler.badRequest(`${user.name} already in a team`));
    const result = await userService.updateUser(userId, { team: teamId });
    return result.modifiedCount != 1
      ? next(ErrorHandler.serverError(`Failed To Add ${user.name} in team`))
      : res.json({
          success: true,
          message: `Successfully added ${user.name} in team`,
        });
  };
  ...
  removeMember = async (req, res, next) => {
    ...
    return result.modifiedCount != 1
      ? next(
          ErrorHandler.serverError(`Failed To Remove ${user.name} from team`)
        )
      : res.json({
          success: true,
          message: `Successfully removed ${user.name} from team`,
        });
  };
  ...
  addRemoveLeader = async (req, res, next) => {
    ...
    if (user.type !== "leader")
      return next(ErrorHandler.badRequest(`${user.name} is not a Leader`));
    const team = await teamService.findTeam({ leader: id });
    console.log(team);
    if (type === "add" && team)
      return next(
        ErrorHandler.badRequest(
          `${user.name} is already leading '${team.name}' team`
        )
      );
    if (type === "remove" && !team)
      return next(
        ErrorHandler.badRequest(`${user.name} is not leading any team`)
      );
    const update = await teamService.updateTeam(teamId, {
      leader: type === "add" ? id : null,
    });
    console.log(type === "add" ? id : null);
    return update.modifiedCount !== 1
      ? next(
          ErrorHandler.serverError(
            `Failed To ${type.charAt(0).toUpperCase() + type.slice(1)} Leader`
          )
        )
      : res.json({
          success: true,
